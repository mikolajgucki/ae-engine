...
// AEEngine: commented out as onDestroy() gets blocked.
// SDL_SemWait(Android_ResumeSem) right after android_egl_context_backup()
// in Android_PumpEvents() is never released even though the semaphore is
// posted in Java_org_libsdl_app_SDLActivity_nativeQuit().
// SDL_SemWait(Android_ResumeSem) right after android_egl_context_backup()
// is called on pause (Java_org_libsdl_app_SDLActivity_nativePause()).
