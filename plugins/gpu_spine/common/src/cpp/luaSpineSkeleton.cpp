...
-- @return `true` on success, `false` if there is no such bone.
*/
...
// fill matrix
    matrix->matrix->set(
        bone->a,bone->b,0,bone->worldX,
        bone->c,bone->d,0,bone->worldY,
        0,0,1,0,
        0,0,0,1);
...
/*
-- @name :getRegionWorldXYCoords
-- @func
-- @brief Gets world XY coordinates of a region.
-- @param name The attachment name.
-- @return The region coordinates as table.
*/
static int skeletonGetRegionWorldXYCoords(lua_State *L) {
    SpineSkeletonType *data = checkSpineSkeletonType(L);
    const char *name = luaL_checkstring(L,2);
...
// get coordinates
    SpineUtil spineUtil;
    if (spineUtil.getRegionWorldXYCoords(data->skeleton,name,xy) == false) {
        luaPushError(L,spineUtil.getError().c_str());
        return 0;
    }
    
// table
    LuaTableUtil luaTableUtil;
    luaTableUtil.arrayToTable(L,xy,8);
...
static int skeletonUpdate(lua_State *L) {
    ...
    spSkeleton_update(data->skeleton,seconds);
...
static int skeleton__tostring(lua_State *L) {
    lua_pushstring(L,"ae.spine.Skeleton []");
    return 1;
}
...
static const struct luaL_Reg skeletonMethods[] = {
    ...
    {"getRegionWorldXYCoords",skeletonGetRegionWorldXYCoords},
...
/** */
void loadSpineSkeletonLib(LuaEngine *luaEngine,DLog *log_) {
    dlog = log_;
